   ...
   function delete1(po_id){
	   $.post(
		   ...
		   function(data){
			   if(data=="1"){
				   alert("删除成功！");
				   location.href="../wupin/weifahuo.do";
			   }
		   }
		   );
...
slide ck-next">下一张</a>
  ...
  <div class="main" >
   ...
   <c:forEach items="${frontpo3}" var="po" varStatus="status">
   <!-- 一件物品 --> 
    ...
    <!-- 一件物品 -->    
         ...
         <!--分页-->
               ...
               <!-- 如果当前页是第一页，不能点上一页 -->
               <c:choose>
                	<c:when test="${frontpoPage3.currentPage==1}">
        	        <a href="#">上一页</a>
        	        </c:when>
        	        <c:otherwise>
                			<a href="../wupin/weifahuo.do?currentPage=${frontpoPage3.currentPage-1}">上一页</a>
                	</c:otherwise>
                </c:choose>
        	     <!-- 循环输出页面，从1开始到 totalPage结束-->   
                  <c:forEach begin="1" end="${frontpoPage3.totalPage}" var="p">
                    <!-- 如果是当前页有样式，否则为没有样式 -->
                     <c:choose>
                         <c:when test="${p==frontpoPage3.currentPage}">
                             <a href="../wupin/weifahuo.do?currentPage=${p}" class="current_page">${p}</a>
                         </c:when>
                         <c:otherwise>
                              <a href="../wupin/weifahuo.do?currentPage=${p}">${p}</a>
                         </c:otherwise>
                     </c:choose>
                  </c:forEach>
                  ...
                  <!--  如果当前页是最后一页，不能点下一页 -->
                  <c:choose>
                    <c:when test="${frontpoPage3.currentPage==frontpoPage3.totalPage}">
                       <a href="#">下一页</a>
                    </c:when>
                    <c:otherwise>
                       <a href="../wupin/weifahuo.do?currentPage=${frontpoPage3.currentPage+1}">下一页</a>
                    </c:otherwise>
                  </c:choose>
                </div>
             <!-- 分页 -->
