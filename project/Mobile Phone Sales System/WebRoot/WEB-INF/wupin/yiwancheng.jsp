   ...
   function delete1(po_id){
	   $.post(
		   ...
		   function(data){
			   if(data=="1"){
				   alert("删除成功！");
				   location.href="../wupin/yiwancheng.do";
			   }
		   }
		   );
      ...
      function tuihuo(po_id){
	  var agree=confirm("确定退货么?");
...
slide ck-next">下一张</a>
  ...
  <div class="main" >
   ...
   <c:forEach items="${frontpo1}" var="po" varStatus="status">
   <!-- 一件物品 --> 
    <div  style="width:867px;float:left;margin-left:10px;margin-top:10px;border:1px solid #c5c5c5;" onmouseover="this.style.border='1px solid #8d8b8b'" onmouseout="this.style.border='1px solid #c5c5c5'">
       <div style="float:left;width:867px;height:30px;background-color:#d5d3d3;">
         ...
         <div style="float:right;margin-top:7px;margin-right:20px;">
            <a href="javascript:delete1(${po.po_id});"><img title="删除" style="cursor:pointer;" src="../front-images/lajixiang.jpg"/></a>
         </div>
       </div>
       <div>
         <table style="height:50px;margin-left:20px;">
            <tr>
               ...
               <td style="width:100px;" align="center">
                  <a id="tuihuo"  href="javascript:tuihuo(${po.po_id})"><span style="font-size:14px;color:red;">${po.tuihuo}</span></a>
               </td>
            </tr>
         </table>
       </div>
    ...
    <!-- 一件物品 -->    
         ...
         <!--分页-->
               ...
               <!-- 如果当前页是第一页，不能点上一页 -->
               <c:choose>
                	<c:when test="${frontpoPage1.currentPage==1}">
        	        <a href="#">上一页</a>
        	        </c:when>
        	        <c:otherwise>
                			<a href="../wupin/yiwancheng.do?currentPage=${frontpoPage1.currentPage-1}">上一页</a>
                	</c:otherwise>
                </c:choose>
        	     <!-- 循环输出页面，从1开始到 totalPage结束-->   
                  <c:forEach begin="1" end="${frontpoPage1.totalPage}" var="p">
                    <!-- 如果是当前页有样式，否则为没有样式 -->
                     <c:choose>
                         <c:when test="${p==frontpoPage1.currentPage}">
                             <a href="../wupin/yiwancheng.do?currentPage=${p}" class="current_page">${p}</a>
                         </c:when>
                         <c:otherwise>
                              <a href="../wupin/yiwancheng.do?currentPage=${p}">${p}</a>
                         </c:otherwise>
                     </c:choose>
                  </c:forEach>
                  ...
                  <!--  如果当前页是最后一页，不能点下一页 -->
                  <c:choose>
                    <c:when test="${frontpoPage1.currentPage==frontpoPage1.totalPage}">
                       <a href="#">下一页</a>
                    </c:when>
                    <c:otherwise>
                       <a href="../wupin/yiwancheng.do?currentPage=${frontpoPage1.currentPage+1}">下一页</a>
                    </c:otherwise>
                  </c:choose>
                </div>
             <!-- 分页 -->
