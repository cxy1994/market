...
<script type="text/javascript">
   ...
   function delete1(po_id){
	   ...
	   var r = window.confirm("确定要删除未完成订单么？删除后将无法恢复");
    	if(r==true){
    		  $.post(
		   "deletewupin.do",
		   {"po_id":po_id},
		   function(data){
			   if(data=="1"){
				   alert("删除成功！");
				   location.href="../wupin/weifukuan.do";
			   }
		   }
		   );
   ...
   function fukuan(po_id){
	  var agree=confirm("确定要付款吗 ?");
...
slide ck-next">下一张</a>
  ...
  <div class="main" >
   ...
   <c:forEach items="${frontpo2}" var="po" varStatus="status">
   <!-- 一件物品 --> 
       ...
       <div>
         <table style="height:50px;margin-left:20px;">
            <tr>
               ...
               <td style="width:100px;" align="center">
                 <a  href="javascript:fukuan(${po.po_id});"> <span style="font-size:14px;">${po.fukuan}</span></a>
               </td>
               <td style="width:100px;" align="center">
    ...
    <!-- 一件物品 -->    
         ...
         <!--分页-->
               ...
               <!-- 如果当前页是第一页，不能点上一页 -->
               <c:choose>
                	<c:when test="${frontpoPage2.currentPage==1}">
        	        <a href="#">上一页</a>
        	        </c:when>
        	        <c:otherwise>
                			<a href="../wupin/weifukuan.do?currentPage=${frontpoPage2.currentPage-1}">上一页</a>
                	</c:otherwise>
                </c:choose>
        	     <!-- 循环输出页面，从1开始到 totalPage结束-->   
                  <c:forEach begin="1" end="${frontpoPage2.totalPage}" var="p">
                    <!-- 如果是当前页有样式，否则为没有样式 -->
                     <c:choose>
                         <c:when test="${p==frontpoPage2.currentPage}">
                             <a href="../wupin/weifukuan.do?currentPage=${p}" class="current_page">${p}</a>
                         </c:when>
                         <c:otherwise>
                              <a href="../wupin/weifukuan.do?currentPage=${p}">${p}</a>
                         </c:otherwise>
                     </c:choose>
                  </c:forEach>
                  ...
                  <!--  如果当前页是最后一页，不能点下一页 -->
                  <c:choose>
                    <c:when test="${frontpoPage2.currentPage==frontpoPage2.totalPage}">
                       <a href="#">下一页</a>
                    </c:when>
                    <c:otherwise>
                       <a href="../wupin/weifukuan.do?currentPage=${frontpoPage2.currentPage+1}">下一页</a>
                    </c:otherwise>
                  </c:choose>
                </div>
             <!-- 分页 -->
