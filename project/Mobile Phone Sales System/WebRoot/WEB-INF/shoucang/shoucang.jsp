...
<script type="text/javascript">
   ...
   //购物车自增自减     
    ...
    function onmouseover1( ){
     ...
     function shanchu(shoucang_id){
    	var r = window.confirm("确定要删除此物品么？");
    	if(r==true){
    		
    		location.href="../shoucang/deleteshoucang.do?shoucang_id="+shoucang_id;
    	}
...
slide ck-next">下一张</a>
  ...
  <div class="main" >
    ...
    <div style="margin:0 auto;width:400px;">
         <img src="../front-images/shoucangjia.png" style="height: 100px;width:400px;"/>
    </div>
    
    <div style=" width:970px;">
        <table  id="tab" width="970px" style="border:1px solid #1999fe; border-spacing:0px;">
          <tr height="30px" >
               <td colspan="5" style=" background-image:url('../front-images/bg1.jpg');"></td>
          </tr>
           <tr align="center">
               <td style="border:1px solid #1999fe;width:150px;">商品名称</td>
               <td style="border:1px solid #1999fe;width:100px;">商家</td>
           ...
           <c:forEach items="${shouCangList}" var="s" varStatus="var">
           <tr align="center" >
               <td style="border:1px solid #1999fe;width:150px;" id="phonename">${s.phoneName}</td>
               <td style="border:1px solid #1999fe;width:100px;" id="username">${s.buserName}</td>
               <td style="border:1px solid #1999fe;width:350px;">
                  ${s.chicun};${s.memery};${s.cpu};${s.fenbianlv};${s.front};${s.behind};
               </td>
               <td style="border:1px solid #1999fe;width:100px;" > 
                   <span class='price'  id="price">￥:${s.phone_price}</span>
               </td>
               <!-- 
               <td style="border:1px solid #1999fe;width:100px;">
               ...
               <td style="border:1px solid #1999fe;width:200px;" >
                    <a  href="#" onclick="shanchu(${s.shoucang_id});"><img style="width:60px;" src="../front-images/删除.png"/></a>
               </td>
           </tr>
           </c:forEach>
          <tr>
         ...
         <tr>
             <td colspan="5" align="center" style="background-image:url('../front-images/bg2.jpg')">
                 <!--分页-->
                <div id="pages" style="width:330px;height:30px;">
               <!-- 如果当前页是第一页，不能点上一页 -->
               <c:choose>
                	<c:when test="${shouCangPage.currentPage==1}">
        	        <a href="#"><span style="color:graytext;">上一页</span></a>
        	        </c:when>
        	        <c:otherwise>
                			<a href="../shoucang/shoucang.do?currentPage=${shouCangPage.currentPage-1}"><span style="color:graytext;">上一页</span></a>
                	</c:otherwise>
                </c:choose>
        	     <!-- 循环输出页面，从1开始到 totalPage结束-->   
                  <c:forEach begin="1" end="${shouCangPage.totalPage}" var="p">
                    <!-- 如果是当前页有样式，否则为没有样式 -->
                     <c:choose>
                         <c:when test="${p==shouCangPage.currentPage}">
                             <a href="../shoucang/shoucang.do?currentPage=${p}" class="current_page">${p}</a>
                         </c:when>
                         <c:otherwise>
                              <a href="../shoucang/shoucang.do?currentPage=${p}">${p}</a>
                         </c:otherwise>
                     </c:choose>
                  </c:forEach>
                  ...
                  <!--  如果当前页是最后一页，不能点下一页 -->
                  <c:choose>
                    <c:when test="${shouCangPage.currentPage==shouCangPage.totalPage}">
                       <a href="#"><span style="color:graytext;">下一页</span></a>
                    </c:when>
                    <c:otherwise>
                       <a href="../shoucang/shoucang.do?currentPage=${shouCangPage.currentPage+1}"><span style="color:graytext;">下一页</span></a>
                    </c:otherwise>
                  </c:choose>
